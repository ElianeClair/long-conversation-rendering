...
本文是[长对话的开窗优化：从30秒到2秒](rendering.zh-CN.md)的分支篇。通用的渲染优化（`content-visibility`、分层懒加载、滚动校正）在那篇展开，这里只讲树形结构特有的部分。
...
在线性对话中，编辑一条消息 = 替换那条消息的DOM（[swapMsgEl](rendering.zh-CN.md#定点dom替换)）。在树形对话中，编辑一条消息的语义完全不同——它在编辑位置创建了一条新分支：
...
最后是两条容易被忽略的默认前提。**钉底假设"最新的在底部"**——聊天式界面的习惯。自上而下的文档式界面把钉底换成钉顶，思路不变。**"编辑=开新分支"（第4节）是产品决定，不是渲染逻辑的要求**——如果你的编辑是原地改、不保留旧分支，那就回到[正篇的 swapMsgEl](rendering.zh-CN.md#定点dom替换)，两套互相兼容。
